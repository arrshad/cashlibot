...
import type { BadgeStatus, Lang } from '@/types';

export function StatsBody() {
  const lang = useAppStore((s) => s.me!.language_code);
        ...
        <>
          <div className="glass-card level-card">
            <div className="level-card-row">
              <span className="level-card-label">{t(lang, 'stats.level_label')}</span>
...
function BadgeTile({ badge, lang }: { badge: BadgeStatus; lang: Lang }) {
